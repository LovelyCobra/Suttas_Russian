...
def extract_sutta_info(soup: BeautifulSoup) -> Dict[str, str]:
    ...
    transl_elems = soup.find_all('div', align='right')#.get_text()
    for elm in transl_elems:
        if 'Перевод' in clean_text_content(elm.get_text()): 
            transl_elem = elm
            print(sutta_num)
            break
        else:
            transl_elem = None
    # print(str(transl_elem))
    # print(transl_elem.get_text())
    if transl_elem:# and 'Перевод с пали: ' in transl_elem and 'источник' in transl_elem:
        translator = re.search(r'Перевод.+(?=источник)', clean_text_content(transl_elem.get_text()), re.DOTALL).group()
        # print(translator)
    else:
        translator = 'Перевод: см. сайт www.theravada.ru'
...
def extract_sutta_content(soup: BeautifulSoup) -> str:
    ...
    td_children = [tag for tag in td_children if tag.name != 'table']
...
def sutta_title_html(pali_title, russ_title, sutta_number, translator='Перевод с пали: А. Я. Сыркина') -> str:
    return f"""
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <td>
            ...
            <font size="4" face="Times New Roman, Times, serif"><i>{pali_title}</i>:<br>
                ...
                <br>
                <font size="2"><i>{translator}</i></font>
            </font>
            </div>
        </td>
        </tr>
...
def create_epub(output_filename: str = "digha_nikaya.epub") -> None:
    directory = 'Дигха Никая с ударениями'
    ...
    book.add_metadata('DC', 'description', 'Russian translation of Long Discourses of the Buddha with stress marks')
    ...
    with open('Russ_suttas/Digha-cover_stressed_.jpg', 'rb') as cover_file:
    ...
    stress_chapter = epub.EpubHtml(title='Stress Page', file_name='stress.xhtml', lang='ru')
    stress_chapter.content = (
        '<div style="text-align: center; margin-top: 15%;">'
    ...
    book.add_item(stress_chapter)

    acknowledgments_chapter = epub.EpubHtml(title='Acknowledge Page', file_name='acknowledge.xhtml', lang='ru')
    acknowledgments_chapter.content = (
        '<div style="text-align: center; margin-top: 15%;">'
        '<p>If you have enjoyed this e-book or have found it useful or helpful, please seriously consider to express your appretiation by making a donation to:<br><br></p>'
        '<h3>www.theravada.ru</h3>'
        '<p>see the options at <br><b>https://theravada.ru/About/about.htm</b></p>'
        # '<br>'
        '<p><i>A N D</i></p>'
        # '<br>'
        '<h3>www.russiangram.com</h3>'
        '<p>see the donation button at <br><b>https://russiangram.com</b><br><br><br></p>'
        '<p><i>The practice of <b>dāna</b> (generosity, giving), after all, is the integral part of the path to liberation.</i></p>'
        '</div>'
    )
    book.add_item(acknowledgments_chapter)

    chapters = []
    toc = []
    print(col.SEP)
    print(f"Processing Digha Nikaya suttas...")
    for sutta_num in sorted(sutta_groups.keys(), key=int):
        ...
        translator = sutta_info['translator']
        ...
        full_html = sutta_title_html(pali_title, russian_title, sutta_number, translator) + '\n'.join(combined_html)
    ...
    book.spine = ['cover', title_chapter, stress_chapter, acknowledgments_chapter,  'nav'] + chapters
...
if __name__ == "__main__":
    output_filename = 'Дигха Никая с ударениями_.epub'
    create_epub(output_filename)

    # Дигха Никая с ударениями.epub