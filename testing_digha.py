...
def extract_sutta_content(soup: BeautifulSoup) -> str:
    ...
    if 'Содержание' in first_subheading.get_text():
    ...
    divs = content_td.find_all('div')
    divs_a = [div for div in divs if div.get('class') == ['a']]
...
def create_epub(hrefs: List[str], output_filename: str = "majjhima_nikaya.epub") -> None:
    ...
    book.add_metadata('DC', 'source', 'Pali Canon')
    ...
    title_chapter.content = (
        ...
        '<h3>Перевод с английского: А.Я.Сыркин & SV</h3>'
        '<p><i>источник: Pali Canon</i></p>'
    ...
    big_sections = [
        ("О нравственности", 1, 13),
        ("Большой раздел", 14, 17),
        ("Сутты Патики", 18, 19)
    ]
    subgroup_sizes = {
        50: [10] * 5,
    ...
    for big_name, start, end in big_sections:
        ...
        big_toc_entries = []
        current_num = start
        for n, size in enumerate(tqdm(sizes, desc="Working on suttas:", ascii=True, color='green')):
...
#         hrefs.insert(70, "https://theravada.ru/Teaching/Canon/Suttanta/Texts/mn71-tevijja-vacchagotta-sutta.htm")
#         hrefs.insert(72, "https://theravada.ru/Teaching/Canon/Suttanta/Texts/mn73-maha-vacchagotta-sutta.htm")
#         hrefs.insert(73, "https://theravada.ru/Teaching/Canon/Suttanta/Texts/mn74-dighanakha-sutta.htm")
...
if __name__ == "__main__":
    ...
    # url = 'https://theravada.ru/Teaching/Canon/Suttanta/Texts/mn5-anangana-sutta-sv.htm'

    # session = create_session()
    # content = fetch_page_content(session, url)
    # if content:
        # file_name = re.search('mn.+?sv.htm$', url).group()
        # with open(f'Russ_suttas/{file_name}l', 'w', encoding='utf-8') as f:
        #     f.write(content)
        # print(f'{col.SEP}The page have been successfuly fetched and saved!{col.SEP}')
        # soup = BeautifulSoup(content, 'lxml')
        # extract_sutta_content(soup)


    # sutta_links = sutta_list("'мадджхима-hикая'")
    # print(col.SEP)

    # q = 20
    # problems = []
    # failed = []
    # for link in tqdm(sutta_links[20:], desc="Checking note number:", ascii=True, colour="green"):
    #     session = create_session()
    #     content = fetch_page_content(session, link)
    #     if content:
    #         soup = BeautifulSoup(content, 'lxml')
    #         note_number = extract_sutta_content(soup)
    #         if note_number%3 != 0:
    #             problems.append(q)
    #     else:
    #         failed.append(q)
    #     q += 1
    # print(f"Problematic suttas: {problems}\nSuttas failed to fetch:{failed}{col.SEP}")